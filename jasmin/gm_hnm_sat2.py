...
jobids0=['xnwda','xnwdb','xnwdc','xnwdd','xnwde','xnwdf']
jobid_base='xnfiy'
...
def plot_on(number,baseline_t,baseline_var,jobids,njobs,var,loc,areas,yy):
	...
	for i in range(njobs):
		...
		base_file=Dataset(loc+jobids0[i]+'/netcdf/'+jobids0[i]+'_atmos.nc')
		anomaly_var=anomaly_file.variables[var][:120,:,:].squeeze()
		base_var=base_file.variables[var][:120,:,:].squeeze()
		anomaly_t=anomaly_file.variables['t'][:120]
		anomaly_file.close()
		base_file.close()
		anomaly_var=anomaly_var-base_var
		nts=len(anomaly_t)
