...
jobid_base='xnfiy'
#jobid_base='xnwdg'
jobids=['xnofa','xnofg','xnofn','xnoft','xnofb','xnofh','xnofo','xnofu','xnofc','xnofi','xnofp','xnofv','xnofd','xnofj','xnofq','xnofw','xnofe','xnofk','xnofr','xnofx','xnoff','xnofl','xnofs','xnofy']
...
baseline_var=baseline_file.variables[var][:744,:,:].squeeze()
baseline_t=baseline_file.variables['t'][:744]
lons=baseline_file.variables['longitude'][:]
lats=baseline_file.variables['latitude'][:]
lsmfile=Dataset(loc+'/lsms/'+'/pi_gem.nc')
...
for i in range(njobs):
	...
	if METHOD==1:
		...
		store_anom[:,i,:,:]=anomaly_var[:48,:,:]
                print anomaly_var.shape
                print baseline_var.shape
		delta=anomaly_var-baseline_var[base_loc:base_loc+nts,:,:]
	elif METHOD==2:
		delta=np.zeros([48,len(lats),len(lons)])
		for l in range(48):
