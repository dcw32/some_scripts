...
outname='gl_rads_timeseries_SW'+time.strftime("%Y%m%d")
save=True
format='png'
################################################################
# BEGIN
...
def plot_on_toa(ax,jobids,njobs,loc,areas,col,lab,verbose,mm):
	...
	if verbose==True:
		...
		ax.plot(t_plot[:mm],0.-store0-store1,c='k',linewidth=2.,label='Net')
                ...
                ax.plot(t_plot[:mm],0.-store0,c=col,linewidth=2.,label=lab)
def plot_on_surf(ax,jobids,njobs,loc,areas,col,lab,verbose,mm):
	...
	if verbose==True:
        	...
        	ax.plot(t_plot[:mm],store2+store3-store4-store5,c='k',linewidth=2.,label='Net')
                ...
                ax.plot(t_plot[:mm],store2,c=col,linewidth=2.,label=lab)
plot_on_toa(ax0,jobids1,njobs,loc,areas1,cols[0],labelm[0],False,120)
plot_on_toa(ax0,jobids2,njobs,loc,areas1,cols[1],labelm[1],False,120)
plot_on_toa(ax0,jobids3,njobs,loc,areas1,cols[2],labelm[2],False,120)
plot_on_toa(ax0,jobids4,njobs,loc,areas1,cols[3],labelm[3],False,120)
plot_on_surf(ax1,jobids1,njobs,loc,areas1,cols[0],labelm[0],False,120)
plot_on_surf(ax1,jobids2,njobs,loc,areas1,cols[1],labelm[1],False,120)
plot_on_surf(ax1,jobids3,njobs,loc,areas1,cols[2],labelm[2],False,120)
plot_on_surf(ax1,jobids4,njobs,loc,areas1,cols[3],labelm[3],False,120)
...
ax0.axhline(0.,c='k')
ax1.axhline(0.,c='k')
ax0.legend(fancybox=True,ncol=4,handletextpad=0.1,columnspacing=0.2,loc='lower center',bbox_to_anchor=(1.0,1.01))
ax1.yaxis.tick_right()
ax1.yaxis.set_ticks_position('both')
ax0.text(0.02,1.02,'B',transform=ax0.transAxes\
...
ax1.text(0.95,0.05,'Surface',transform=ax1.transAxes\
                        ,verticalalignment='bottom',horizontalalignment='right'\
                        ,fontsize='14',color='k')
plt.figtext(0.08,0.5,r"Change to Shortwave Radiative Budget / W m$\mathregular{^{-2}}$"\
         ,verticalalignment='center',horizontalalignment='center'\
         ,fontsize='12',color='k',rotation='90')
#plt.figtext(0.5,0.03,"Years After Eruption"\
#         ,verticalalignment='center',horizontalalignment='center'\
#         ,fontsize='12',color='k')
#ax0.legend(fancybox=True,ncol=3,loc='lower right')
if save==True:
