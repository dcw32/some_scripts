...
def budget(jobid):
        ...
        secf1=file.variables['cond_oc_nuc_sol'][:12,:,:,:]
        secf2=file.variables['cond_oc_ait_sol'][:12,:,:,:]
        secf3=file.variables['cond_oc_acc_sol'][:12,:,:,:]
        secf4=file.variables['cond_oc_coa_sol'][:12,:,:,:]
        secf5=file.variables['cond_oc_ait_ins'][:12,:,:,:]
        file.close()
        file=Dataset('/scratch/dcw32/netscratch/um/'+jobid+'/'+jobid+'_prim_oc.nc')
#       prim1=file.variables['prim_oc_to_ait_sol'][:]
        prim2=file.variables['prim_oc_to_ait_ins'][:12,:,:,:]
        file.close()
